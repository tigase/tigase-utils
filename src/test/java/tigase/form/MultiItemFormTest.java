...
//		System.out.println( multiItemForm.getElement().toStringPretty() );
		...
		Assert.assertEquals( title, multiItemForm.getElement().findChildStaticStr( new String[] {"x", "title"} ).getCData());

		Assert.assertNotNull( multiItemForm.getElement().findChildStaticStr( new String[] {"x", "instructions"} ) );
		Assert.assertEquals( instructions, multiItemForm.getElement().findChildStaticStr( new String[] {"x", "instructions"} ).getCData());

		Assert.assertNotNull( multiItemForm.getElement().findChildStaticStr( new String[] {"x", "reported"} ) );
		Assert.assertEquals( 1, multiItemForm.getElement().findChildStaticStr( new String[] {"x", "reported"} ).getChildren().size());
...
//		System.out.println( multiItemForm.getElement().toStringPretty() );
		...
		Assert.assertNotNull( multiItemForm.getElement().findChildStaticStr( new String[] {"x", "reported"} ) );
		Assert.assertEquals( reported.getAllFields().size(),
												 multiItemForm.getElement().findChildStaticStr( new String[] {"x", "reported"} ).getChildren().size());
